...
uv run pytest -q               # 473 tests, including a randomised sweep
uv run lint-imports             # billing logic must not import the database
node --test tests/frontend/*.test.js   # 202 frontend tests
...
```
http://192.168.1.101:5500/member.html?as=周恆
```
...
```
http://localhost:5500/member.html?as=周恆        # a member's view
http://localhost:5500/organizer.html?as=周恆     # the same person, managing
http://localhost:5500/member.html?as=Jason       # somebody else entirely
```
