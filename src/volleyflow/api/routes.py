...
def _get_or_create_player(db: Session, club_id: int, name: str) -> PlayerRow:
    ...
    in two different clubs is two different people (per the design rules:
...
def _require_self_or_organizer(
    ...
    organizer" shape the attendance and billing rules describe for absences, signups,
...
def _reject_if_already_playing(
    ...
    # A fixed member is expected at every game already;
...
def _expected_on_court(db: Session, game: GameRow, season: SeasonRow) -> int:
    ...
    as attending by default — see the attendance rules, "members are expected by
    default."
...
def _games_with_no_room(db: Session, season: SeasonRow) -> list[GameRow]:
    ...
    full of drop-ins, and nothing checked. The attendance rules make capacity a
...
def _require_season_open(season: SeasonRow) -> None:
    ...
    The billing rules: settlement "computes each member's absence refund and
...
def _make_room_for_substitute(
    ...
    away, so by the time the member names the person they
...
def _promote_from_waitlist(db: Session, game_id: int) -> int | None:
    ...
    The attendance rules: "a member records an absence -> the waitlist is offered
...
def _offer_freed_slots_to_the_queue(db: Session, season: SeasonRow) -> list[int]:
    ...
    queue for it. The attendance rules list an absence and a cancelled signup
...
def start_season(
    ...
    # A fixed member holds a slot for the whole season, so
...
def update_season(
    ...
    # share_per_game's denominator, so moving any of them
...
def add_member(
    ...
    # the capacity-based split the season fee assumes
...
def get_season(
    ...
    return SeasonDetailOut(
        ...
        # missed when the rule changed on 2026-09-10, so
...
def set_substitute(
    ...
    allowed to arrange this per the attendance rules.
    ...
    # A fixed member is expected at this game already, so
...
def record_payment(
    ...
    """A manual cash movement the organizer marks by hand — the billing rules:
