...
def test_join_unknown_club_returns_404(client: TestClient) -> None:
    # With a genuine token for that id: a link to a club since deleted.
    # Without one it is refused before existence is looked at (see
    # test_invites.py), so a stranger can't learn which ids exist.
    carol = identify(client, "Carol")

    response = client.post(
        "/clubs/999999/join",
        json={"invite": invite_token(999999)},
...
def test_update_season_can_clear_a_nullable_field_back_to_null(
    ...
    season = _start_season(client, member_names=["Alice"], location="晴光館")
...
def test_a_stranger_cannot_join_a_club_as_someone_they_are_not(
    ...
    response = client.post(
        ...
        # An invite, so the request gets past the door; and a player_id,
        # which the route has no field for and must ignore.
        json={"invite": invite_token(club["id"]), "player_id": 999999},
        headers=auth_headers(carol["token"]),
    )
...
# leave, who dropped in, and what a game costs. The design rules say a club
...
def test_a_club_cannot_be_read_by_somebody_outside_it(
    client: TestClient,
) -> None:
    """GET /clubs/{id} used to answer anyone signed in, for an invite
    prompt that needed a name before joining. That prompt reads
    GET /invites/{token} now — see test_invites.py — and a club's name by
    its id is for its members, or anybody could list every club by trying
    ids in order."""
    ...
    assert response.status_code == 403
    assert "晴光館" not in response.text


# --- deleting things -----------------------------------------------------
#
# Test clubs and mistaken seasons need to be removable, but settled books
# do not: the design rules want every data change auditable, and destroying
...
def test_link_moves_the_line_identity_onto_the_roster_entry(
    ...
    assert [m["id"] for m in members if m["name"].startswith("陳品妍")] == [typed_in_id]
...
def test_after_linking_that_person_can_act_as_themselves(client: TestClient) -> None:
    ...
    response = client.post(
        "/absences",
        json={"player_name": "陳品妍", "game_id": season["games"][0]["id"]},
...
def test_link_carries_over_the_guests_that_account_signed_up(
    ...
    assert [row["brought_by"] for row in brought] == ["陳品妍"], (
...
def test_the_air_conditioning_correction_is_an_adjustment_not_an_edit(
    ...
    # The billing rules: never edit what someone was already charged.
    season = _start_season(
        client,
        total_venue_cost="10000",
        game_dates=["2026-08-04", "2026-08-11"],
