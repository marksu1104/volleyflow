  ...
  return {
    season: {
      ...
      members: [
        ...
        { id: 3, name: "翔", gender: "male", linked: true },
        { id: 4, name: "阿Joy", gender: "female", linked: false },
      ],
    },
    game: {
      id: 10,
      ...
      locked: false,
      absences: [
        { id: 100, player_name: "林書妤", covered_by: "Jason" },
        { id: 101, player_name: "翔", covered_by: null },
      ],
      confirmed_drop_ins: [
        {
          id: 200,
      ...
      waitlist_entries: [{ id: 300, player_name: "芷若", gender: "female" }],
    },
  };
}
...
function render(options = {}) {
  ...
  renderGameDetail(el, season, game, { viewerName: "周安", ...options });
  return el.innerHTML;
}
...
test("the waitlist keeps its order and marks the viewer", () => {
  ...
  game.waitlist_entries = [
    { id: 300, player_name: "芷若", gender: "female" },
    { id: 301, player_name: "周安", gender: "male" },
  ];
  const el = makeElement();
...
test("without a waitlist handler the queue shows no remove button", () => {
  const { season, game } = fixture();
  const el = makeElement();
  renderGameDetail(el, season, game, { viewerName: "周安", onRemoveDropIn() {} });
...
test("a group with nobody in it still gets a tab, showing zero", () => {
  ...
  renderGameDetail(el, season, quiet, { viewerName: "周安" });
...
test("only the organizer is offered 遞補 on a queued person", () => {
  ...
  renderGameDetail(asMember, season, game, { viewerName: "周安", onLeaveWaitlist() {} });
...
test("somebody who signed themselves up is 臨打, not anybody's 代打", () => {
  const { season, game } = fixture();
  game.confirmed_drop_ins = [
    { id: 200, player_id: 9, player_name: "Momo", gender: "male", covering: null, linked: false },
  ];
  game.absences = [
    { id: 100, player_name: "林書妤", covered_by: null, filled_by: "Momo" },
  ];
  const el = makeElement();
  ...
  assert.doesNotMatch(el.innerHTML, /代 林書妤/, "he agreed to no such thing");
  // Named, because "who took my slot" is the first thing asked — but
  // 已補上, not 代打: he was never asked to stand in for her.
  assert.match(el.innerHTML, /Momo 已補上/);
  assert.doesNotMatch(el.innerHTML, /Momo 代打/);
});

test("an absence nobody is filling is still a gap", () => {
  const { season, game } = fixture();
  game.confirmed_drop_ins = [];
  game.absences = [{ id: 100, player_name: "林書妤", covered_by: null, filled_by: null }];
  const el = makeElement();
...
test("a tap is never swallowed while an earlier change is in flight", () => {
  ...
  const tap = () =>
    el.onclick({
      target: {
        closest: (s) =>
          s === "[data-mark-absent]" ? { dataset: { markAbsent: "阿Joy" } } : null,
      },
    });
...
// seasonHasStarted draws the line the roster rules are
...
test("a settled season's sheet can still be read", () => {
  ...
  assert.match(settled, /Jason/);
  assert.match(settled, /芷若/);
});
