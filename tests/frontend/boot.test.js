...
test("identify retries a cold start rather than giving up on the first try", async () => {
  ...
  globalThis.fetch = async () => {
    ...
    return { ok: true, json: async () => ({ id: 7, name: "周安" }) };
  };
  globalThis.liff = {
    init: async () => {},
    isLoggedIn: () => true,
    getIDToken: () => "tok",
    getProfile: async () => ({ displayName: "周安" }),
  };

  const identified = await initLiffIdentity("http://x", "liff-id");

  assert.equal(calls, 3);
  assert.deepEqual(identified, { id: 7, name: "周安" });
});
...
test("?as= only works on a laptop, never on the real site", () => {
  ...
  globalThis.location = { hostname: "localhost", search: "?as=周恆" };
  assert.equal(devIdentityName(), "周恆");

  globalThis.location = { hostname: "marksu1104.github.io", search: "?as=周恆" };
...
test("the identity sticks across pages without carrying the parameter", () => {
  const { devIdentityName } = load();
  globalThis.location = { hostname: "127.0.0.1", search: "?as=林書妤" };
  devIdentityName();

  globalThis.location = { hostname: "127.0.0.1", search: "" };
  assert.equal(devIdentityName(), "林書妤");
});
...
test("a dev identity is sent as a token nothing else could produce", () => {
  ...
  assert.equal(authHeader().Authorization, "Bearer dev:%E5%91%A8%E6%81%86");
});
...
test("a phone on the wifi is allowed to sign in as somebody", () => {
  ...
  const { devIdentityName } = load();
  globalThis.location = { hostname: "192.168.1.101", search: "?as=周恆" };
