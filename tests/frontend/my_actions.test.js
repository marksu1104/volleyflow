...
test("a member who took leave can undo it", () => {
  const { season, game } = fixture({
    game: { absences: [{ id: 100, player_name: "周安", covered_by: null }] },
  });
  as("周安");
...
test("a member whose leave is covered can cancel the substitute", () => {
  ...
  const { season, game } = fixture({
    game: {
      absences: [{ id: 100, player_name: "周安", covered_by: "Jason" }],
      confirmed_drop_ins: [
        { id: 200, player_name: "Jason", covering: "周安", gender: "male" },
      ],
    },
  });
  as("周安");
...
test("signing people up is offered in every state", () => {
  ...
  const states = [
    fixture(),
    fixture({ game: { absences: [{ id: 1, player_name: "周安", covered_by: null }] } }),
    fixture({
      game: {
        absences: [{ id: 1, player_name: "周安", covered_by: "Jason" }],
        confirmed_drop_ins: [{ id: 2, player_name: "Jason", covering: "周安" }],
      },
    }),
  ];
...
test("a substitute's name is escaped — it was typed by a person", () => {
  const { season, game } = fixture({
    game: {
      absences: [{ id: 100, player_name: "周安", covered_by: '<img src=x onerror=alert(1)>' }],
      confirmed_drop_ins: [
        { id: 200, player_name: "x", covering: "周安", gender: null },
      ],
    },
  });
  as("周安");
...
test("every control in the row is the same kind of button", () => {
  ...
  const { season, game } = fixture({
    game: { absences: [{ id: 1, player_name: "周安", covered_by: null }] },
  });
  as("周安");
