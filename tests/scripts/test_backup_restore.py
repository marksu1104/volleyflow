"""backup_db.py / restore_db.py: the restore drill that answers
...
the real dev branch, the same way the 2026-09-12 dry run
did by hand.
"""
...
def _seed(session: Session) -> None:
    ...
    session.add(club)
    session.add(PlayerRow(id=1, name="周恆"))
...
def test_restores_a_real_disaster_on_neon(tmp_path: Path) -> None:
    ...
    manual dry run of 2026-09-12: back up, delete
