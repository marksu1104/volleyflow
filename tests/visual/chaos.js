...
(async () => {
  ...
  const solo = await open(browser, "organizer.html", "周恆");
  ...
  const flipper = await open(browser, "member.html", "周恆");
  const takeLeave = `(() => {
  ...
  const organizer = await open(browser, "organizer.html", "周恆");
  // 阿凱 rather than a name off the roster: a roster entry the organizer
  ...
  // scripts/seed_dev.py.
  const member = await open(browser, "member.html", "阿凱");
  await Promise.all([
  ...
  await money.goto(`${BASE}/organizer-ledger.html?as=${as("周恆")}`, {
